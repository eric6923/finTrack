  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Error creating business shares! Status: ${response.status} - ${error.message}`);
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="max-w-4xl w-full mx-auto p-6 bg-white shadow-md rounded-lg">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6">
            <label className="block text-gray-700 font-medium mb-2">Number of Shareholders:</label>
            <input 
              type="number" 
          ...
          <div className="mb-6">
            {shareholders.map((shareholder, index) => (
              <div key={index} className="mb-4 p-4 border rounded-md bg-gray-50">
                <h3 className="text-lg font-semibold mb-4">Shareholder {index + 1}</h3>
                <div className="mb-4">
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-2">Share Percentage:</label>
                  <input 
                    type="number" 
                    value={shareholder.sharePercentage} 
                    onChange={(e) => handleShareholderChange(index, 'sharePercentage', Number(e.target.value))} 
                    required 
                    min="0" 
                    max="100" 
                    className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
