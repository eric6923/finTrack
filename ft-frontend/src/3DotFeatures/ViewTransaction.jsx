    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        if (log.payLaterDetails?.busId) {
          ...
          const bus = responseBus.data.find(
            (item) => item.id === log.payLaterDetails.busId
          );
        ...
        if (log.collection?.operatorId) {
          const responseOperator = await axios.get(
            "https://ftbackend.vercel.app/api/user/operator",
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
          const operator = responseOperator.data.find(
            (item) => item.id === log.collection.operatorId
          );
        ...
        if (log.commission?.agentId) {
          ...
          const agent = responseAgent.data.find(
            (item) => item.id === log.commission.agentId
          );
                  ...
                  <td colSpan="2" className="py-2 px-4 border-b">
                    <div className="overflow-y-auto max-h-48">
                      ...
                      <div className="space-y-2">
                        ...
                        <p>
                          <strong>Bus Name:</strong> {busName}
                        </p>
                      </div>
                    </div>
                  </td>
